...
app.post('/login', (req, res)=>{
    ...
    let query0 = `SELECT dept_work, user_type, user_name, user_id FROM Employees INNER JOIN Users ON Employees.emp_no = Users.user_id WHERE Users.email = '${userEmail}' AND Users.password = '${userPass}'`;
    ...
    let queryResult0 = mssql.query(query0, (err0, result0)=>{
        ...
        else if(result0.recordset.length > 0 && Object.values(result0.recordset[0]).length == 4){
            // console.log('query0 value is 2');
            let dept = Object.values(result0.recordset[0])[0];
            let privilege = Object.values(result0.recordset[0])[1];
            let userName = Object.values(result0.recordset[0])[2];
            let userID = Object.values(result0.recordset[0])[3];
...
app.post('/getChartData1', (req, res)=>{
        ...
        let b = 1;
        for(let i = a; i >=b; i--){
            let query = `SELECT count(*) AS '${count}' FROM History INNER JOIN Employees ON Employees.emp_no=History.emp_id INNER JOIN department ON department.dept_name=Employees.dept_work where dept_work='${req.body.dept}' and SUBSTRING(movement_time,1,10) = '${moment().subtract(count, 'days').format('DD/MM/YYYY')}'`;
            // console.log(query);
...
app.post('/getChartData2', (req, res)=>{
    ...
    for(let i = a; i >=b; i--){
        ...
        let query = `SELECT count(*) AS '${count}' FROM Alert INNER JOIN assets ON Alert.tag_id = assets.tag_id INNER JOIN department ON assets.dept_id = department.dept_id WHERE dept_name = '${req.body.dept}' AND SUBSTRING(date, 1, 10) = '${moment().subtract(count, 'days').format('DD/MM/YYYY')}'`;
        // console.log(query);
        let day = moment().subtract(count, 'days').format('ddd');
        labelArr.push(day);
        count++;
...
app.post('/mdeny', (req, res)=>{
    ...
    let queryResult1 = mssql.query(q1, (err1, result1)=>{
        if(err1) {
            // console.log('failed deny query1')
        }
        else{
            res.send('Denied');
        }
...
//     // let query = `SELECT  asset_id , asset_name , location_name FROM Assets INNER JOIN location ON Assets.location_id = location.location_id WHERE emp_no = '${req.body.dept}'`;
//     let query = `SELECT  asset_id , asset_name , location_name FROM Assets INNER JOIN location ON Assets.location_id = location.location_id WHERE emp_no = '1003'`;
//     let queryResult = mssql.query(query, (err, result)=>{
//         if(err) throw err
//         else{
//             res.send(result);
...
// app.post('/setMovementsTable', (req, res)=>{
//     // let query = `SELECT TOP(20) FROM History WHERE emp_id = '${req.body.empID}'`;
//     let query = `SELECT TOP(20) FROM History WHERE emp_id = '1003'`;
//     let queryResult = mssql.query(query, (err, result)=>{
//         if(err) throw err
//         else{
//             res.send(result);
...
app.post('/setAssetsTable', (req, res)=>{
    ...
    let queryResult = mssql.query(query, (err, result)=>{
        if(err){
            // console.log('/setAssetsTable err');
        }
        else{
            // console.log('1');
...
app.post('/setMovementsTable',(req, res)=>{
    let userID = req.body.userID;
    // console.log(userID);
    let query = `SELECT TOP(20) * FROM History WHERE emp_id = '${userID}'`;
    let queryResult = mssql.query(query, (err, result)=>{
        if(err){
            // console.log('/setMovementsTable err');
        }
        else{
            // console.log('2');
            res.send(result.recordset);
...
app.post('/setUserCards', (req, res)=>{
    // console.log(1);
    let userID = req.body.userID;
    let startDate = moment().add(0, 'days').format('DD/MM/YYYY');
    let endDate = moment().subtract(7, 'days').format('DD/MM/YYYY');
    let arr = [];
    let query0 = `SELECT COUNT(*) FROM Assets WHERE emp_no= '${userID}'`;
    let query1 = `SELECT dept_work FROM Employees WHERE emp_no = '${userID}'`;
    let query2 = `SELECT COUNT(*) FROM History  WHERE  emp_id = '${userID}' AND SUBSTRING(movement_time, 4, 2) = '${moment().format('MM')}'`;
    let query3 = `SELECT COUNT(*) FROM History WHERE emp_id = '1001' AND SUBSTRING(movement_time, 1, 10) BETWEEN '${startDate}' AND '${endDate}'`;
...
app.post('/setRequestCards', (req, res)=>{
    let userID = req.body.userID;
    let arr = [];
    let query0 = `SELECT COUNT(*) FROM Movement_request INNER JOIN assets ON Movement_request.asset_id = assets.asset_id WHERE emp_no = '${userID}'`;
    let query1 = `SELECT COUNT(*) FROm Movement_request INNER JOIN assets On MOvement_request.asset_id = assets.asset_id WHERE emp_no = '${userID}' AND Request_status = 'Approved'`;
    let query2 = `SELECT COUNT(*) FROm Movement_request INNER JOIN assets On MOvement_request.asset_id = assets.asset_id WHERE emp_no = '${userID}' AND Request_status = 'Denied'`;
    let query3 = `SELECT COUNT(*) FROm Movement_request INNER JOIN assets On MOvement_request.asset_id = assets.asset_id WHERE emp_no = '${userID}' AND Request_status = 'Pending'`;
...
app.post('/setAlertsCard', (req, res)=>{
    let userID = req.body.userID;
    let query = `SELECT COUNT(*) FROM Alert INNER JOIN assets ON Alert.tag_id = assets.tag_id WHERE emp_no = '${userID}'`;
    let queryResult = mssql.query(query, (err, result)=>{
        if(err) throw err
        else{
...
app.post('/userDetails',(req, res)=>{
    ...
    // res.send(['Sarthak', 'Kumar', 'Rath', 'Electronics', 'Admin', '1234567890', 'Sarthak@gmail.com']);
})
...
app.post('/resetPass', (req, res)=>{
    ...
    let query = `UPDATE Users SET password = '${newPass}' WHERE user_id = '${userID}'`;
    // console.log(query);
...
// console.log(moment().subtract(3, 'days').format('DD/MM/YYYY'));
